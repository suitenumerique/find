...
class IndexBulkError(IndexerError):
    ...
    def __init__(self, message, action=None, _id=None, params=None):
        super().__init__(message, action, params)
        self.action = action
        self.params = params
        self.id = _id
...
class IndexerTaskService:
    ...
    def process_content(self, document, content):
        ...
        try:
            ...
            converter = self.converters.get(document.mimetype)
            ...
            raise IndexContentError("Unable to convert document data") from e
    ...
    def preprocess_all(self, batch_size=None):
        ...
        for doc in docs:
            ...
            except (requests.HTTPError, IndexerError) as e:
                errors[str(doc.id)] = {"status": "error", "message": str(e)}
    ...
    def load_all(self, batch_size=None):
        ...
        for doc in docs:
            try:
                response = requests.get(doc.content_uri, stream=True, timeout=10)
                response.raise_for_status()

                content = self.process_content(doc, response.raw)
    ...
    def embed_all(self, model_name=None):
        """
    ...
    def index(self, documents):
        ...
        of their content.
        Returns the errors from the commit
        """
        with openbulk(
